...
// Define global constant memory for SWAParams
__constant__ SWAParams d_params;

// Kernel functions to compute the score matrix in parallel
__global__ void smithWatermanKernel_P1(int *scoreMatrix, const char *seqA, const char *seqB, const int antidiagDim, const int scoreMatrixDim)
{
    ...
    {
        // Match or mismatch?
        int matchScore = (seqA[j - 1] == seqB[i - 1]) ? d_params.match : d_params.mismatch;
        int scoreDiag = scoreMatrix[(j - 1) * scoreMatrixDim + (i - 1)] + matchScore;
        // Gap from North
        int scoreUp = scoreMatrix[(j - 1) * scoreMatrixDim + i] + d_params.gap;
        // Gap from West
        int scoreLeft = scoreMatrix[j * scoreMatrixDim + (i - 1)] + d_params.gap;

        scoreMatrix[j * scoreMatrixDim + i] = max(0, max(scoreDiag, max(scoreUp, scoreLeft)));
    }
}

__global__ void smithWatermanKernel_P2(int *scoreMatrix, const char *seqA, const char *seqB, const int antidiagDim, const int scoreMatrixDim)
{
    int thread_id = blockIdx.x * blockDim.x + threadIdx.x + 1; // Global thread ID
...
SWAResult smithWaterman(const std::string &seqA, const std::string &seqB, const int seqLen, SWAParams params)
{
    ...
    cudaMemcpyToSymbol(d_params, &params, sizeof(SWAParams));
    ...
    for (ii = 1; ii < scoreMatrixDim; ii++)
    {
        ...
        smithWatermanKernel_P1<<<blocks, numThreads>>>(d_scoreMatrix, d_seqA, d_seqB, ii, scoreMatrixDim);
    }
    ...
    for (int jj = scoreMatrixDim; jj > 0; jj--)
    {
        ...
        smithWatermanKernel_P2<<<blocks, numThreads>>>(d_scoreMatrix, d_seqA, d_seqB, ii, scoreMatrixDim);
        ii++;
    }
